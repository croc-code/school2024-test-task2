...
## Автор решения
Косолобов Дмитрий Сергеевич

## Описание реализации
Сначала данные десериализуются из JSON файла с помощью библиотеки Gson

Для каждого заказа, оформленного в декабре, программа анализирует содержимое корзины и подсчитывает количество товаров в каждой категории.
Этот анализ выполняется с помощью метода getCategoriesAndCount, который возвращает HashMap, содержащую пары "категория - количество".

Полученная таблица сортируется по количеству товаров в каждой категории.

Затем программа определяет самую популярную категорию (или категории, если их несколько с одинаковым количеством товаров).
Для этого из отсортированной таблицы извлекается наиболее популярная категория, а затем извлекаются все категории с таким же количеством товаров.

Отчет формируется в формате JSON и выводится в стандартный вывод.
Если популярных категорий товаров больше одной, они выводятся все в алфавитном порядке.

## Инструкция по сборке и запуску решения
В коммандной строке выполните следующие команды:
1. git clone https://github.com/DmitriyKosolobov/school2024-test-task2.git
2. cd school2024-test-task2/application
3. mvn exec:java -Dexec.mainClass="edu.croc.Task" 
