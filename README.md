...
## Описание реализации
Решение включает в себя создание трех сущностей: Order, Item и Category для более удобного преобразования json в java объекты.
Также для удобства преобразования в решении используется библиотека jackson.
Был создан функциональный интерфейс - OrderParser, который включает в себя один метод - parse.
Для определения популярной категории(категорий) я создал класс OrderCategoriesFilter, а в нем метод getMostPopularCategoriesFromDecemberOrders(), который принимает путь к json файлу, а также OrderParser, который я в последствии реализую с помощью лямбда выражения.
В классе TestTask я создаю список наиболее популярных категорий с 1 по 31 декабря с момощью всех вышеперечесленных инструментов. В качестве одного из аргументов метода getMostPopularCategoriesFromDecemberOrders() я реализую лямбда выражение, в котором прописываю логику для парсинга json.  
В конце использую утилитарынй класс, который соддержит в себе метод возвращения категорий в праавильном формате, т.е. формате json.
